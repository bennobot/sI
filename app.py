...
if st.session_state.header_data is not None:
    ...
    tabs.append("🚀 Finalize & Export PO") # Always show final tab
    ...
    # --- TAB 3: HEADER / EXPORT ---
    # Logic to find correct index: If missing tab exists, this is index 2. Else index 1.
    final_tab_idx = 2 if not all_matched else 1
    
    with current_tabs[final_tab_idx]:
        st.subheader("3. Finalize & Export")
        
        if not all_matched:
            st.warning("⚠️ You have unmatched products. You can export, but unmatched lines will be skipped.")
        else:
